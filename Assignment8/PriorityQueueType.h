...
template<class ItemType> void PQType<ItemType>::MakeEmpty() {
    length = 0;
}
...
template<class ItemType> bool PQType<ItemType>::IsFull() const {
    return length == maxItems;
}

// Post: Returns true if the queue is empty; false, otherwise.
template<class ItemType> bool PQType<ItemType>::IsEmpty() const {
    return length == 0;
}